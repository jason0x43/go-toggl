...
type DetailedReportParams struct {
	UserAgent   string    `json:"user_agent"`
	WorkspaceID int       `json:"workspace_id"`
	Since       time.Time `json:"since"`
	Until       time.Time `json:"until"`
	Billable    string    `json:"billable"` // yes, no, both
	Page        int       `json:"page"`
...
func (rp DetailedReportParams) merge(rawParams map[string]string) (merged map[string]string) {
	merged = rawParams
	...
	for i := 0; i < v.NumField(); i++ {
		...
		case "UserIDs":
			skip = true
			// valueString = strings.Join(value.Interface().([]int), ",") //string(value.Int())
		case "TagIDs":
			valueString = arrayToString(value.Interface().([]int), ",")
			//valueString = strings.Join(value.Interface().([]string), ",") //string(value.Int())
		case "Rounding":
			if value.Bool() {
				valueString = "on"
			} else {
...
func (session *Session) GetDetailedReportV2(rp DetailedReportParams) (DetailedReport, error) {
	params := rp.merge(map[string]string{
		"user_agent": "jc-toggl",
		"page":       "1",
...
// GetTagByName get tag by name.
func (session *Session) GetTagByName(name string, wid int) (tag Tag, err error) {
	tags, err := session.GetWorkspaceTags(wid)
	if err != nil {
		return tag, err
	}
...
func (session *Session) GetWorkspaceTags(wid int) (tags []Tag, err error) {
	...
	path := fmt.Sprintf("/workspaces/%d/tags", wid)
	respData, err := session.get(TogglAPI, path, data)
	if err != nil {
		return tags, err
	}
